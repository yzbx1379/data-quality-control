...
[数据集交付模板/](./数据集交付模板/) 目录收录三类数据集（代码问答 / 安全技术博客 / 漏洞修复 commit）的**完整交付物料清单与 `metadata.csv` 统计口径模板**。

交付包一次性提供全部物料，**缺失任意文件不予验收**，每类数据集均包含四件：

1. **核心数据**（分片 / 分批次 JSONL）；
2. **元数据统计表 `metadata.csv`**（总样本数、语言 / 来源 / 领域分布、脱敏覆盖、全局重复率等统计维度）；
3. **数据质检报告**（本仓库质检脚本的输出，含人工复检结论）；
4. **README.md**（采集流程、清洗去重、匿名化规范、字段释义、版权合规与使用限制）。

各统计维度与硬指标阈值的对应关系、以及物料与本仓库质检脚本的对应关系，详见 [数据集交付模板/README.md](./数据集交付模板/README.md)。
